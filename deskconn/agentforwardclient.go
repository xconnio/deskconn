...
func runAgentForwardQUIC(ctx context.Context, realm, cfgDirectory, authID, agentSock string, ready chan<- error) error {
	...
	defer stream.Close()
	if err := common.WriteMsg(stream, common.RoutingFrame{Realm: realm, Op: common.FSOpAgentForward}); err != nil {
		ready <- err
		return err
	}
...
func startAgentForward(conn shellConn, sendKey, receiveKey []byte, authID string) error {
	env, err := common.BuildPortEnvelope(common.AgentFwdMsgControl,
		common.MustJSON(common.AgentForwardMsg{Op: common.AgentFwdOpStart, AuthID: authID}), sendKey)
	if err != nil {
		return err
	}
...
func runAgentForwardClientLoop(ctx context.Context, readNext func() (byte, []byte, error),
	writer *common.PortReverseWriter, sendKey []byte, agentSock string) error {
	var mu sync.Mutex
	...
	for {
		...
		case common.AgentFwdMsgControl:
			...
			switch msg.Op {
			case common.AgentFwdOpConnect:
				connID := msg.ConnID
				localConn, dialErr := net.Dial("unix", agentSock)
				if dialErr != nil {
					if env, encErr := common.BuildPortEnvelope(common.AgentFwdMsgControl,
						common.MustJSON(common.AgentForwardMsg{Op: common.AgentFwdOpClose, ConnID: connID}), sendKey); encErr == nil {
						writer.Send(env)
					}
					continue
				}
				...
				mu.Unlock()
				common.SafeGo(func() { pumpAgentForwardLocalConn(localConn, connID, sendKey, writer, &mu, localConns) })
			case common.AgentFwdOpClose:
				mu.Lock()
...
// closes.
func pumpAgentForwardLocalConn(localConn net.Conn, connID uint64, sendKey []byte, writer *common.PortReverseWriter,
	mu *sync.Mutex, localConns map[uint64]net.Conn) {
	buf := make([]byte, 32*1024)
	for {
		n, readErr := localConn.Read(buf)
		if n > 0 {
			env, encErr := common.BuildPortEnvelope(common.AgentFwdMsgData, common.EncodeConnData(connID, buf[:n]), sendKey)
