...
`), 0600))
	...
	config, err := loadStandaloneConfig(cfgDir)
	require.NoError(t, err)
	require.True(t, config.Enabled)
	require.Equal(t, standaloneDefaultListen, config.Listen)
	require.Equal(t, []deskconn.StandalonePrincipal{{AuthID: testAuthID, AuthorizedKeys: []string{"k1"}}},
		config.Principals)
}
...
func TestLoadStandaloneConfigEnvOverrides(t *testing.T) {
	...
	require.Equal(t, standaloneAuthRole, principals[1].AuthRole)
}

func TestLoadStandaloneConfigDisabledByDefault(t *testing.T) {
	config, err := loadStandaloneConfig(t.TempDir())
	require.NoError(t, err)
...
func TestLoadStandaloneConfigInvalidEnv(t *testing.T) {
	...
	_, err := loadStandaloneConfig(t.TempDir())
	require.Error(t, err)
...
func TestLoadOrCreateCertIsStable(t *testing.T) {
	...
	first, err := loadOrCreateCert(cfgDir)
	require.NoError(t, err)
...
func TestStandaloneRelaysAuthenticatedStream(t *testing.T) {
	...
	require.NoError(t, err)
	config := &deskconn.StandaloneConfig{
		Listen:     "127.0.0.1:0",
		Principals: []deskconn.StandalonePrincipal{{AuthID: testAuthID, AuthorizedKeys: []string{pubKey}}},
	}
	...
	listener, _, stop, err := startStandalone(cfgDir, config)
	require.NoError(t, err)
	t.Cleanup(stop)
	...
	connect := func(privateKey string) (*xconn.QUICSession, error) {
		...
		require.NoError(t, err)
		return xconn.ConnectQUIC(context.Background(), listener.Addr().String(), deskconn.StandaloneRealm,
	...
	require.NoError(t, err)
	require.NoError(t, deskconn.WriteMsg(stream, deskconn.RoutingFrame{Realm: deskconn.StandaloneRealm,
		Op: deskconn.FSOpShell}))
	...
	header, err := deskconn.ReadRelayHeader(relayed)
	require.NoError(t, err)
	require.Equal(t, deskconn.RelayKindQUIC, header.Kind)
	require.Equal(t, deskconn.FSOpShell, header.Op)
