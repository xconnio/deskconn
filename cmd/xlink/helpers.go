...
func bridgeHandler(appSession *xconn.Session, procedure string) xconn.InvocationHandler {
	return func(_ context.Context, inv *xconn.Invocation) *xconn.InvocationResult {
		...
		resp := call.Do()
		if resp.Err != nil {
			return xconn.NewInvocationError(deskconn.ErrOperationFailed, resp.Err.Error())
		}
...
func RegisterBridge(session *xconn.Session, appSession *xconn.Session) error {
	for _, procedure := range deskconn.AppLayerProcedures {
...
func EnsureCredentials() (*deskconn.Credentials, error) {
	credFilePath, err := deskconn.CredentialsFilePath()
	if err != nil {
		return nil, err
	}
