...
func LoadStandaloneConfig(cfgDirectory string) (*common.StandaloneConfig, error) {
	...
	if v := os.Getenv("DESKCONN_STANDALONE_KEYS"); v != "" {
		for entry := range strings.SplitSeq(v, ",") {
			...
			standalone.Principals = append(standalone.Principals,
				common.StandalonePrincipal{AuthID: authid, AuthorizedKeys: []string{key}})
		}
	}
...
func standalonePrincipals(config *common.StandaloneConfig) []*CryptosignPrincipal {
...
func RunStandalone(cfgDirectory string, config *common.StandaloneConfig) {
	...
	fingerprint := common.CertFingerprint(cert.Certificate[0])
	log.Printf("standalone mode: listening on %s/udp (realm %s)", listener.Addr(), common.StandaloneRealm)
...
func startStandalone(cfgDirectory string, config *common.StandaloneConfig) (*xconn.QUICListener,
	...
	router := NewDeviceRouter(common.StandaloneRealm)
	...
	common.SafeGo(func() {
		for stream := range listener.AcceptStream() {
			common.SafeGo(func() { RelayQUICStream(stream.Conn, xlinkStreamSock) })
		}
	})
