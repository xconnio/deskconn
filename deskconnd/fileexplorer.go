...
func (f *FileBrowser) Browse(pathArg, cursor string, limit int) (*common.FileBrowseResult, error) {
	...
	all := make([]common.FileEntry, 0, len(entries))
...
func fileEntrySortKey(e common.FileEntry) string {
	dirRank := "1"
	if e.IsDir {
		dirRank = "0"
	}
...
func generateThumbnails(entries []common.FileEntry) {
...
func buildFileBrowseResult(homeDir, path string, info os.FileInfo) *common.FileBrowseResult {
	result := &common.FileBrowseResult{
		Path:      path,
...
func buildFileEntry(path string, info os.FileInfo) common.FileEntry {
...
func isVideoFile(name string) bool {
	switch strings.ToLower(filepath.Ext(name)) {
	case common.ExtMp4, common.ExtWebm, common.ExtMov, common.ExtAvi, common.ExtMkv, common.ExtOgv, common.ExtFlv,
		common.ExtWmv:
		return true
	}
	return false
...
func (f *FileBrowser) Search(pathArg, query string, showHidden bool, sendKey []byte, inv *xconn.Invocation) error {
	...
	batch := make([]common.FileEntry, 0, searchBatchSize)
...
func sendSearchBatch(inv *xconn.Invocation, sendKey []byte, entries []common.FileEntry) error {
	plaintext, err := json.Marshal(entries)
	if err != nil {
		return err
	}
	encrypted, err := common.EncryptPayload(plaintext, sendKey)
	if err != nil {
		return err
	}
