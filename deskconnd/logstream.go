...
func sendLogData(send logSender, sendKey []byte, data []byte) bool {
	env, err := common.BuildPortEnvelope(common.LogMsgData, data, sendKey)
	if err != nil {
		return false
	}
	return send(env) == nil
...
func (d *Deskconn) handleQUICLogsStream(stream net.Conn) {
	...
	if err != nil || kind != common.LogMsgControl {
		return
	}
	var ctrl common.LogControlMsg
	...
	send := newLogSender(func(envelope []byte) error { return common.WriteFrame(stream, envelope) })
	...
	common.SafeGo(func() {
		for {
			...
			_, _, _ = common.DecryptEnvelope(frame, receiveKey)
		}
	})
...
func (d *Deskconn) serveLogsChannel(channel common.MessageChannel, firstMessage []byte) {
	...
	common.SafeGo(func() {
		for {
			if _, err := common.RecvPriority(msgCh, closed, common.ShellIdleTimeout); err != nil {
				_ = channel.Close()
				return
			}
		}
...
func streamLogsRaw(send logSender, sendKey []byte, done <-chan struct{}, ctrl common.LogControlMsg) {
	if ctrl.Since != "" {
