...
func (s *IndexService) Start(ctx context.Context) {
	s.once.Do(func() {
		common.SafeGo(func() { s.runWatcher(ctx) })
		if !s.ready.Load() {
			common.SafeGo(func() { s.runIndexer(ctx) })
		} else {
			common.SafeGo(func() { s.runSync(ctx) })
		}
	})
}
...
func fileCategory(name string) string {
	...
	case common.ExtJpg, common.ExtJpeg, common.ExtPng, common.ExtGif, common.ExtBmp, common.ExtWebp,
